...
- **targetLanguage**: set on L0 (`sdd_initialize_system`), override per L1 — enables language-aware validation (foreign builtins are flagged).
...
- **Narratives (L5, inside L4)**: numbered steps, `type: local | call`; a `call` step names `targetComponent` + `targetMethod`, which must exist on a declared dependency's interface. Granular edits (insert/delete/update steps, reopen status) go through `sdd_update_spec`.
