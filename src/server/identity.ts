...
import { appendAuditEvent, queryAuditEvents as repoQueryAuditEvents, pruneAuditEvents as repoPruneAuditEvents, DEFAULT_AUDIT_POLICY } from './audit.js';
...
//
// Pure forwarding to the orchestrator functions above. Phase 1 decision: these
// routes ride the ADMIN-plane listener (127.0.0.1 bind) until HostExposurePolicy
// / `identityApiEnabled` lands in a later phase to give the identity plane its own
// exposure switch. The credential is extracted the same way admin routes do (the
// caller passes the bearer token). Endpoints match iidentity_portal exactly.
