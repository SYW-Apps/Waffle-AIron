...
export function addDomain(domain: Domain): void {
  ...
  if (registry.domains.some((d) => d.id === domain.id)) {
    throw new WaffagentError(`Domain "${domain.id}" already exists.`);
  }
  if (registry.domains.some((d) => d.path === domain.path)) {
    throw new WaffagentError(`A domain for path "${domain.path}" already exists.`);
  }
...
export function updateDomain(updated: Domain): void {
  ...
  if (index === -1) throw new WaffagentError(`Domain "${updated.id}" not found.`);
...
export function removeDomain(id: string): void {
  ...
  if (index === -1) throw new WaffagentError(`Domain "${id}" not found.`);
...
// Domain scaffolding
//
// When a domain is added, scaffold its local .ai/ and agent output directories.
// ---------------------------------------------------------------------------
...
/**
 * Write a CLAUDE.md to the domain directory with standard waffagent instructions,
 * including the job handoff pickup protocol.
 *
 * The relative path from the domain directory back to the root .ai/jobs/ is
...
function writeDomainClaudeMd(domain: Domain, domainAbsPath: string): void {
  ...
  const jobsPath = `${relativeToRoot}/.ai/jobs`;

  const content = `# ${domain.name} — Agent Instructions

This directory is a managed domain in the waffagent topology.

## Agent Files

AI agent definitions for this domain live in \`.claude/agents/\`.
They are generated by waffagent — do not edit them directly.
To update an agent: modify the registry at the root project's
\`.ai/registry/agents.json\`, then run \`waffagent generate\`.

## Job Handoff Protocol

When this session was started by \`waffagent delegate\`, a job file
was written by the parent agent. If the environment variable
\`WAFFAGENT_JOB_FILE\` is set, read that file immediately on startup
...
\`WAFFAGENT_RESULT_FILE\` (if set), or to
...
- You are a specialized agent for this domain. A parent project agent
  may invoke you via \`waffagent delegate\` for focused tasks.
`;
