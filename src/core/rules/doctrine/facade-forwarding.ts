...
// ---------------------------------------------------------------------------
// The facade rule (docs/standards/architecture.md §7): a Repository's facade
// does PURE 1:1 forwarding — every facade method's authored narrative is
// exactly one `call` step targeting an owned member block. More than one step,
// a non-call step, or a call that leaves the pattern means the facade contains
// logic, and logic belongs inside a member (Registry/Index/Query/Store). Scoped
// to the Repository, the backend pattern whose facade semantics §7 defines;
...
export const facadeForwardingRule: SddRule = {
  ...
  check(ctx) {
    for (const impl of ctx.implementations) {
      ...
      if (!comp || comp.componentType !== 'Repository') continue;
