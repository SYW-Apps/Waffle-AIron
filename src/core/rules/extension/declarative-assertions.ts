...
// The assertion shapes are read off the rule context: this rule reaches pack
// data only through ctx.ext, so it never imports the extension loader.
type LoadedAssertion = RuleContext['ext']['assertions'][number];
type AssertionSelector = Extract<LoadedAssertion, { kind: 'forbid-edge' }>['from'];
