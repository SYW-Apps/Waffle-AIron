...
- **Operating Rules**:
  ...
  4. **Delegation**: Delegate implementation via the \`sdd-delegate\` skill — live agent briefs (\`sdd_get_agent_brief\` MCP tool / \`wairon-agent://\` resource) are composed per call and always current; no session restart. Generated agent files are an optional materialized view of the same topology.
