...
// Local scope:   <project-root>/CLAUDE.md  or  <project-root>/GEMINI.md
// ---------------------------------------------------------------------------
...
export function localGuideFilePath(projectRoot: string, targetType: string): string | null {
  if (targetType === 'claude') return path.join(projectRoot, 'CLAUDE.md');
  if (targetType === 'gemini') return path.join(projectRoot, 'GEMINI.md');
  return null;
}
