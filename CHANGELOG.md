...
## Unreleased (minor â€” from v4.0.0)

Post-v4.0.0 fixes and additive capabilities around extension packs, the hosted
server, chained subprojects, and agent-topology scale (merge with `[minor]` â†’
v4.1.0).
...
- Detail sufficiency: `UNNARRATED_COMPLEXITY` (realized cyclomatic complexity â€”
  exact AST grade only â€” over `rules.complexity.maxUnnarratedComplexity`,
...
- Invariant registry: entities declare `invariants:` (anchored via
  `componentClass`); narrative steps assert them via `assertsInvariants` â€”
...
- Codeâ†”spec Level 3 opener: `CALL_STEP_UNREALIZED` â€” every narrative `call`
...
- Event topology: components declare `emits:` / `subscribesTo:`; paired with
  MessageBus endpoint directions â€” `UNCONSUMED_TOPIC`, `UNSOURCED_SUBSCRIPTION`
  (silent on trees with no event edges).
- Guarantee vocabulary: `UNKNOWN_GUARANTEE` â€” a guarantee token on an L3 method
...
- Facade rule now enforced: `FACADE_FORWARDING` â€” a Repository/Gateway facade
  method with an authored narrative that is not exactly one `call` step to an
  owned member (the standard Â§7 claim, previously "mechanically enforceable"
  but unimplemented; dogfooded at zero cost â€” wairon's own 63 narrated facade
  methods all conform).
- **Static integration gate** (docs/design/integration-conformance.md Â§4,
  ...
  module + each direct dependency's â€” any module of the target subsystem for
  cross-subsystem edges; technology-boundary fakes sanctioned), and
  `MISSING_INTEGRATION_SIM` â€” which activates **per subsystem** once its
...
- **Sim path coverage** (`SIM_PATH_UNCOVERED`, Â§4.5): a harness may claim
  coverage with string anchors â€” `"sim:<component>.<method>"` (happy path)
  ...
  expected â€” the step label IS the path's renumber-proof identity. Anchors
...
- `rules.designDepth: components | interfaces | implementations | narratives`
  ...
  default â€” expectation checks below the declared depth are gated; soundness
...
- **Profile edge-deltas**: `ProfileDef.allowedEdges` â€” `{from[], to[],
  ...
  licenses `Specialist â†’ Store`). Cross-subsystem boundary rules and pattern
...
- **Declarative rule assertions** (docs/design/declarative-rule-dsl.md): a
  ...
  rule logic â€” the hosted (declarative-only) pack path finally carries real
...
- **Pack-declarable guarantee tokens**: the guarantee schema is now open
  (`z.string()`); a pack manifest may declare `guarantees: [compensating, â€¦]`
  to extend the vocabulary. The narrativeâ†”contract consistency check
...
- **Symbolic step labels**: a narrative step may declare a `label` anchor, and
  ...
  numbers at write time â€” the stored spec keeps plain numbers. Guards LLM
...
- **`parallel` fan-out/join step + `detach` call flag** (flow algebra
  extension sanctioned by the technology-boundaries design record Â§4; the
  model review's GPU/robotics/backend convergence). A `parallel` header owns
  body `next..endStep`, covered by â‰¥2 contiguous ordered arms
  (`branches: [{step}]`); the join is implicit after `endStep` once ALL arms
  complete â€” `stepGraph()` routes an arm's last step to the join, never into
  ...
  (under-reports across parallel regions â€” conservative direction).
- **Parallel/detach rendering across the diagram surfaces.** The canvas flow
  ...
  flow continues) â€” an arm's last step wires into the join, never into its
  ...
  step's own lane continues normally â€” failure visibly does not propagate.
  ...
  return). The Steps list spells both out (`âˆ¥ parallel â€” arms â€¦`,
  `â‡¢ detached â€” fire & forget`).
...
- FeatureComponent arity relaxed: exactly one Orchestrator + **one or more**
  Views (was exactly one of each) â€” a feature slice with list/detail/form
...
- Honest profile labeling: `lowlevel-os` / `game-ecs` / `realtime-embedded`
  ...
  roadmap) â€” today they enforce only backend-family fencing; the described
...
- Placement notices: `sdd_add_type` (and siblings) explain flat-layout
  ...
  subsystem parameter. `doctor --fix` still performs no flatâ†’nested migration
  (known gap).
...
per-method `symbol` maps are missing. `Store â†’ Registry` edges are now errors
...
carry more reusable, versioned knowledge â€” while wairon's core model stays
...
- **Versioned reusable pattern references.** Packs declare named, versioned
  ...
  `ctx.ext.patterns` â€” the pattern's actual constraints are enforced by its
...
in wairon's own spec tree â€” the pack loader, the conformance rule set as a
proper in-memory **Repository**, and the `packs`/`rules`/`patterns` CLI â€” held to
the same conformance gate as everything else. See `docs/extending-wairon.md`.

### Component variants (new)

A **variant** is a named, base-anchored specialization of a core stereotype â€” a
"kind of `Adapter`/`Specialist`/â€¦" (e.g. a `publisher`) â€” carrying implementation
guidance. It gives components domain vocabulary and, crucially, tells the
**implementer** "this is the same kind as those other components â€” reuse one
...
*not* a variant â€” that stays a method `guarantee`.)

- **A dynamic registry on top of packs.** Variants live outside packs â€” define
...
- **One per component, strictly base-anchored.** A component declares a single
  ...
  component's stereotype â‰  the variant's base, error).
...
- **SSO for self-hosted providers.** The OIDC adapter now resolves each
  provider's endpoints by **explicit overrides â†’ `.well-known` discovery â†’
  ...
  connect** â€” previously it derived `/authorize`+`/token`, which matches neither.
...
- **Split-horizon endpoints.** New `IdentityProviderConfig` overrides
  ...
  **public** front-channel authorize URL pair with a **VPC-internal** back-channel
  token/JWKS URL â€” the common self-hosted-in-a-private-network topology.
...
- **Agent tokens, separate from web login.** A signed-in user can mint a
  **single-project MCP token** for an AI agent â€” self-scoped (no `key:manage`
...
- **Project lifecycle in the UI.** Humans **create / lock / promote / destroy**
  ...
  grant scope), with a scoped project list â€” no longer CLI/loopback-only.
...
- Generation emits agents for **only the current project's own layer** â€” the
  ...
  **10**, and `waffler_core` from 2000+ to **29** â€” each `.claude/agents` (and the
...
- **`generateComponentImplementers` now defaults to `false`** â€” one owner per
...
- The domain-owner agent template now instructs **hierarchical self-division**:
  ...
  further as needed), and â€” for a chained-subproject domain â€” delegate into the
...
- **`generate` now reconciles its output dirs** â€” it prunes agent files that are
...
- Creating a chained subsystem (`sdd_add_subsystem` with a `projectPath`) now
  **fully initializes** the child in the same action (project.yaml + system
  spec), **non-destructively** â€” never overwriting an existing spec tree (this
...
- **Chained subprojects no longer explode when validated from their own
  ...
  cross-tree components) and codeâ†”spec sourcePaths stored relative to the parent
  root cannot resolve â€” previously this produced hundreds of hard errors from
...
spec schema, and library APIs themselves are fully backward compatible â€” nothing
was removed or renamed):

1. **Stricter validation.** The new conformance gate makes `wairon validate`
   stricter â€” a stale L4 `sourcePath` is now a hard error, and `validate --ci`
   surfaces new warnings. A pipeline that was green may go red until the specs
   are reconciled (see *Compatibility & migration*).
2. **Container image rebased debianâ†’alpine, npm removed.** Extension images
...
session surface, and a container CVE scan â€” all fixed:
...
- **Data-plane batch bypass (HIGH):** a JSON-RPC *batch* body let a second,
  ...
  (which inspected only the first message) â€” a read-only token could smuggle a
...
- **Container image:** rebased to `node:24-alpine`, `apk upgrade`, and npm
  ...
  20 high on the previous debian base; size 488 MB â†’ 318 MB.

### Unified web UI (opt-in)

One role-based browser app for the hosted server â€” developers author specs
...
- **View** â€” an embedded live architecture canvas per authorized project
  (the same engine as `wairon diagram --format canvas`).
- **Specs (authoring)** â€” a component index + structured inspector that reads
  ...
  server-side).
- **Admin (control pages)** â€” session-scoped `/web/admin/*` routes over the
...
- **`wairon serve`** â€” run wairon as an HTTP server that hosts many
  **fully-isolated** projects behind one endpoint (the new `sdd_host` subsystem):
  a public **data plane** (`POST /mcp` â€” the `sdd_*` tools over streamable HTTP,
...
- **`wairon host â€¦`** (in-process, no running server needed â€” works over SSH /
...
- **State-scoped lock/promote:** `lock` validates the tree as-complete and writes
  ...
  `StateId` and refuses on any drift â€” never merges to production.
...
- **Diagrams over the admin API** â€” the diagram engine is now a first-class
  ...
  a browser via a short-lived HMAC-**signed** `/view/diagram` link (no bearer â€”
  the capability is in the URL), with generate/download bearer-authed.
- **Producers** (`sdd_producers`) â€” project a spec tree into an external target as
  a one-way, idempotent subsection. Two producers ship, both raw REST with **no new
  dependency**, and both usable **hosted** (`wairon host producer â€¦`,
  `/admin/projects/{id}/producers/*`) and **locally** (`wairon produce <target> --page <id>`,
  credential from flag/env/prompt):
  - **Notion** â€” a "wairon specs" page subtree whose pages carry each component's
    methods + a Mermaid diagram (target-agnostic `DocPage` model).
  - **Miro** â€” the architecture graph rendered onto a board as native shapes +
...
- **Runtime secret store** â€” integration tokens (git, Notion, Miro, signing)
  resolve data-dir store â†’ env, and `wairon host secret set` / `PUT /admin/secrets/{key}`
  set them at runtime, so an integration can be added to a live container without a
  restart.
- **Git-backed projects** (`sdd_git`) â€” a hosted project can relocate its source
  of truth to a git repo (`wairon host git enable --remote â€¦`, or
  `POST /admin/projects/{id}/git`): the container clones it, works on an isolated
  `wairon/work` branch, and `lock` becomes git-aware â€” sync â†’ validate-as-complete
  â†’ promote â†’ **commit + push the working branch**, recording the commit SHA and a
...
- **Extension packs in a hosted container** (`sdd_host` â†’ `pack_orchestrator` +
  `pack_registry`) â€” install architectural profiles and language/platform tables
  into a running container without shell access, at **server-global** scope
  (`GET/PUT/DELETE /admin/packs/{name}`, or `wairon host packs â€¦`) or into one
  **project** (`GET/PUT/DELETE /admin/projects/{id}/packs/{name}`, committed with
  ...
  are **declarative-only** (pure data â€” profiles + language tables); programmatic
  ...
  *lists*. Server-global packs now live on the data volume (`WAIRON_PACKS_DIR`
  ...
  recreation** â€” previously a machine-wide install landed in an ephemeral home dir.
...
- **New rules:**
  - `MUTUAL_SUBSYSTEM_DEPENDENCY` *(warning)* â€” two subsystems depending on
  ...
  - `INVALID_TRUSTED_LINK` *(error)* / `UNUSED_TRUSTED_LINK` *(warning)* â€”
    trusted links must name real peers and correspond to an actual
    cross-subsystem dependency.
  - `GOD_COMPONENT` *(warning)* â€” `dependsOn` fan-out above 8 suggests a
    split or a pattern facade.
  - `LANGUAGE_FOREIGN_BUILTIN` *(warning)* â€” with a declared
...
- **Bug fixes with visible effect:** the unused-detection walk now covers ALL
  ...
  â€” previously invisible unused components/methods may now be reported
  (true positives).

### Codeâ†”spec conformance (new rule families)
...
word-boundary scan as the universal floor â€” every finding carries its
analysis grade `exact | pattern | generic`).

- **`structural-conformance`** â€” every L4 `sourcePath` must resolve to a real
  file inside the project root (`MISSING_SOURCE_FILE`,
  `SOURCE_PATH_ESCAPES_ROOT` â€” *errors*), and every L3 contract method must be
  realized in that file (`UNREALIZED_METHOD`, `MISSING_SOURCE_PATH`,
  `CONFORMANCE_ANALYSIS_SKIPPED` â€” *warnings*). When TypeScript/JavaScript
  files can only be analyzed below exact grade (no `typescript` resolvable
  from the analyzed project or the wairon install), one
  **`CONFORMANCE_DEGRADED`** warning per run makes the degradation visible â€”
...
- **`dependency-conformance`** â€” runtime imports between component-mapped
  ...
  (`UNREALIZED_DEPENDENCY`) â€” both *warnings*. Type-only imports and
  re-export barrels never accuse; cross-subsystem imports are sanctioned by a
  declared edge to the target subsystem's published surface; portalâ†”server
  mounting declared in the portalâ†’server direction is recognized.
...
- Design record: `docs/design/code-spec-conformance.md` (includes the Level 3
  call-graphâ†”narrative sketch).

### Spec schema (additive)

- `targetLanguage` on L0 (system default) and L1 (subsystem override).
- `trustedLinks: [{ subsystem, reason }]` on L1 â€” explicitly sanctioned tight
...
- Structured method **`params: [{ name, type, description?, optional? }]`**
  on L3 methods â€” when present they are the AUTHORITATIVE source for
...
- **Flow steps**: narratives stay a FLAT ordered list (order mimics the code
  lines) and gain 7 step types that jump by step number â€” `branch` (if/else),
...
- **`LANGUAGE_FOREIGN_FLOW`** *(warning, requires `targetLanguage`)* â€”
...
- **Narrative detail dial**: `detail: full | calls-only | intent` per method
  ...
  (Portal/Observer/Adapter â†’ calls-only, Store/Index/Registry â†’ intent,
  logic components â†’ full). `intent` methods specify behavior as an `intent`
  paragraph instead of steps. `MISSING_NARRATIVE` / `INTENT_FLOOR` hold each
  method to its declared (or defaulted) level â€” explicit declarations as
...
- **Schema-validated writes**: every spec save is Zod-validated first â€” a
...
- **`wairon diagram`** â€” Mermaid component diagrams (subsystem subgraphs,
  boundary-hop edges, `owns` containment, public-surface marking) and L5
  narrative â†’ sequence diagrams; `--all` writes the full living-doc set.
- **`wairon diagram --canvas`** â€” interactive single-page HTML canvas
...
- **`wairon diagram --drawio` / `--excalidraw`** â€” editable exports in open
...
- **Canvas 2.0**: SYW / light themes, redesigned toolbar, view levels
  ...
  Export menu (PNG / draw.io / Excalidraw) that uses the CURRENT â€” possibly
  rearranged â€” positions. `--format <fmt>` flag added.
- **Data-coupling overlay**: a **"Data coupling"** toggle overlays dashed edges
  ...
  (its data/model shape) even when there's no logical `dependsOn` â€” derived from
...
- **View-options panel**: the view toggles (Internals, Externals, Data coupling,
  Issues, Rearrange) moved out of the crowded header bar into a **"âš™ View"**
...
- **Scoped C4-style navigation**: every canvas view renders exactly one
  scope's direct children â€” System â†’ top-level subsystems â†’ a subsystem's
  children (nested subsystems + components) â†’ a pattern's members,
...
- **Canvas readability fixes**: (a) with nothing selected, the sidebar now
  ...
  into) instead of always the root system â€” the breadcrumb still walks up to
  the parent; (b) **focus mode** â€” selecting any box lifts its own edges above
  everything and recolours them **by direction** (outgoing "depends on â†’" vs
  incoming "â† used by") while unrelated elements recede, so one block's relations
  ...
  boxes â€” and hovering a port lights the stub edges to the tiles it serves); (c) the **Types ERD degrades
  gracefully on huge systems** â€” above ~400 in-scope types it renders a
  ...
  anyway" override â€” so a 1000+-type system stays interactive yet fully
  navigable â€” and drilling a subsystem's ERD now shows its own types plus only
  ...
  flooded its scope and re-clustered it); (d) **breadcrumbs preserve the active mode** â€” walking up from a
...
- **Layout picker** â€” a **Layout â–¾** menu lets you switch the auto-layout instead
  ...
  **Layered** (the original columns), **Force** (physics relaxation seeded from
  the layered positions â€” untangles crossings, places nodes near their
  ...
  are size-aware presets computed for both the component view and the ERD â€”
  ...
  instead of crossing the whole diagram â€” and overlapping externals are nudged
  ...
  entrypoints-top-left â†’ leaves-bottom-right flow, then widens into landscape.
...
- An external technology is abstracted as a component: an Adapter behind an
  ...
  declares the binding â€” `technologies: [mysql]` â€” and the ownership tree
  becomes the technology's home. New warning-severity, `lint.allow`-able
  rules police the declaration (no hardcoded vendor lists â€” only declared
  tokens are checked):
  - **`TECH_LEAKAGE`** â€” the token referenced in any spec outside the owning
    boundary (prose, ids, `dependsOn` facade bypasses, vendor-shaped types
    in the shared type space).
  - **`VENDOR_NAME_IN_CONTRACT`** â€” the token in ANY L3 identifier surface
    (method names, signatures, params, endpoints), including the owning
    adapter's own contract: the L3 is the swap seam.
  - **`TECH_ON_LOGIC_COMPONENT`** â€” technology bound outside
    Adapter/Store/Registry/Index suggests a missing Adapter wrapper.
  This is also the spec-side hook for future codeâ†”spec conformance checking
...
- Wairon is now a **profile enforcer with a plugin surface**: platform-
  specific profiles and rules are injected from outside â€” a wrapper tool
...
- `.wai/project.yaml` gains `extensions: { packs: [...] }` â€” each entry a
...
- **`wairon packs add | list | remove`** â€” first-class pack installation.
  ...
  (committed â†’ CI and every clone enforce it); `add --global` installs
...
- **Distribution needs no npm**: wrapper products ship a release ZIP (pack
  ...
  standalone wairon binaries â€” see the ZIP recipe in
  `docs/extending-wairon.md`.
- The narrative **flow algebra stays closed** (packs can gate and re-label
  constructs, never inject step kinds â€” reachability analysis and jump
...
- **Wrapper-product template**: `examples/wrapper/` â€” packs (custom profile
  + language table + injected rule), the installer such a product ships
  (`install.js` â†’ `wairon packs add`), an advanced library-embedding demo,
  and a CI-clean **spec-only** demo project (implementation lives on the
  platform; `requireOwnedPaths: false`) â€” guarded by a golden test so the
...
### Per-spec lint suppression â€” `lint.allow`

- Any L1â€“L4 or type spec may declare
  `lint: { allow: [{ code, reason }] }` â€” wairon's `#[allow(...)]`: the
...
- **`HOLLOW_TYPE`** *(warning)* â€” a type with no fields and no methods is a
...
- **Canvas Types view is a real logical ERD now**: boxes render as
  ...
  the field shape (`LineItem[]` â†’ `*`, optional â†’ `0..1`, plain â†’ `1`) as a
  target-end label, and types group into per-subsystem containers (shared
  system-level types in their own box) â€” ready for large trees.
...
error** (`MISSING_SOURCE_FILE` â€” the spec names code that does not exist;
...
0. **`MISSING_SOURCE_FILE`** â€” fix the `sourcePath` to the real file (or
   remove it while the implementation is still design-only; drafts are
   waived). **`UNREALIZED_METHOD`** â€” if the code name legitimately differs
   ...
   **`UNDECLARED_DEPENDENCY`** â€” declare the real collaboration on the
   component that uses it, or route the cross-subsystem hop through the
   target's published portal.

1. **`MUTUAL_SUBSYSTEM_DEPENDENCY`** â€” if the mutual coupling is intentional
   ...
   trustedLinks:
     - subsystem: runtime-runners
       reason: runtime dispatch latency â€” bus round-trip too slow
   ```

   Otherwise break one direction (usually via events over the bus).
2. **`GOD_COMPONENT`** â€” split the workflow or group cohesive collaborators
   behind a Repository/Gateway facade.
3. **New `UNUSED_COMPONENT`/`UNUSED_METHOD` findings** â€” these were always
...
3b. **`MISSING_NARRATIVE` / `INTENT_FLOOR`** â€” methods are now held to their
...
4. Any rule can be tuned per project in `.wai/project.yaml`:
   ...
   Prefer fixing over silencing â€” the overrides exist for deliberate,
...
Initial development series: SDD spec tree (L0â€“L5), conformance validation,
