...
3. **Reproducibility** — running `waffagent generate` twice on the same registry must
...
- **Not an agent runtime** — waffagent does not run agents, orchestrate tasks, or
...
- `waffagent init` — interactive project initialization with target selection
...
- `waffagent analyze`
- `waffagent suggest-topology`
- `waffagent create-agent`
...
- `waffagent create-agent` interactive flow
- `waffagent create-bundle` interactive flow
- Full glob-based ownership overlap detection
- Diff-aware generation (only update changed agents)

### Phase 3 — Topology Analysis
- `waffagent analyze` — scan repo structure and suggest topology
- `waffagent suggest-topology` — identify ownership gaps and coverage issues

### Phase 4 — Split/Merge Workflows
- `waffagent split` — guided agent splitting
- `waffagent merge` — guided agent merging
...
- The final product name may change from "waffagent" — the package name and
